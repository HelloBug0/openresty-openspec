...
static char *ngx_http_upstream_bind_pool_block(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);
static char *ngx_http_upstream_bind_pool_bind(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);

static ngx_addr_t *ngx_http_upstream_get_local_peer(
    ngx_http_upstream_bind_pool_t *bp);
static void ngx_http_upstream_update_bind_peer(
    ngx_http_upstream_bind_pool_t *bp, ngx_addr_t *addr);
...
static ngx_command_t  ngx_http_upstream_commands[] = {
    ...
    { ngx_string("upstream_bind_pool"),
      NGX_HTTP_MAIN_CONF|NGX_CONF_BLOCK|NGX_CONF_TAKE1,
      ngx_http_upstream_bind_pool_block,
      0,
      0,
      NULL },
    ...
    { ngx_string("bind"),
      NGX_HTTP_UPS_BP_CONF|NGX_CONF_1MORE,
      ngx_http_upstream_bind_pool_bind,
      NGX_HTTP_SRV_CONF_OFFSET,
      0,
      NULL },

    { ngx_string("max_retries"),
      NGX_HTTP_UPS_BP_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_num_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_upstream_bind_pool_t, max_retries),
      NULL },
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
    ...
    if (u->conf->bind_pool) {
        ngx_addr_t *addr = ngx_http_upstream_get_local_peer(u->conf->bind_pool);
        if (addr) {
            u->peer.local = addr;
        } else {
             ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                           "no live bind address in pool \"%V\"",
                           &u->conf->bind_pool->name);
             ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
             return;
        }
    } else if (ngx_http_upstream_set_local(r, u, u->conf->local) != NGX_OK) {
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (rc == NGX_ERROR) {
        if (u->conf->bind_pool) {
            ngx_http_upstream_update_bind_peer(u->conf->bind_pool, u->peer.local);

            if (u->bind_pool_tries < u->conf->bind_pool->max_retries) {
                u->bind_pool_tries++;
                u->peer.local = ngx_http_upstream_get_local_peer(u->conf->bind_pool);
                if (u->peer.local) {
                    ngx_http_upstream_connect(r, u);
                    return;
                }
...
static ngx_addr_t *
ngx_http_upstream_get_local_peer(ngx_http_upstream_bind_pool_t *bp)
{
    ngx_uint_t                      i, n, rr;
    ngx_http_upstream_bind_peer_t  *peers;
    time_t                          now;
    ngx_uint_t                      backup = 0;
...
retry:
    ...
    for (i = 0; i < n; i++) {
        ngx_uint_t idx = (rr + i) % n;

        if (peers[idx].backup != backup) {
            continue;
        }

        if (peers[idx].fails >= peers[idx].max_fails) {

            if (now - peers[idx].checked <= peers[idx].fail_timeout) {
                continue;
            }
...
static void
ngx_http_upstream_update_bind_peer(ngx_http_upstream_bind_pool_t *bp,
    ngx_addr_t *addr)
{
    ngx_uint_t                      i;
    ngx_http_upstream_bind_peer_t  *peers;
    ...
    for (i = 0; i < bp->peers.nelts; i++) {
        if (&peers[i].addr == addr) {
            peers[i].fails++;
            peers[i].accessed = ngx_time();
            peers[i].checked = ngx_time();
            break;
        }
    }
}
...
static char *
ngx_http_upstream_bind_pool_block(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    char                             *rv;
    ngx_conf_t                        pcf;
    ngx_str_t                        *value;
    ngx_http_conf_ctx_t              *ctx, *http_ctx;
    ngx_http_upstream_bind_pool_t    *bp, **bpp;
    ngx_http_upstream_main_conf_t    *umcf;
    ...
    bpp = ngx_array_push(&umcf->bind_pools);
    if (bpp == NULL) {
        return NGX_CONF_ERROR;
    }

    bp = ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_bind_pool_t));
    if (bp == NULL) {
        return NGX_CONF_ERROR;
    }

    bp->name = value[1];
    bp->max_retries = NGX_CONF_UNSET_UINT;

    if (ngx_array_init(&bp->peers, cf->pool, 4,
                       sizeof(ngx_http_upstream_bind_peer_t))
        != NGX_OK)
    {
        return NGX_CONF_ERROR;
    }
    ...
    *bpp = bp;
    ...
    ctx->srv_conf[ngx_http_upstream_module.ctx_index] = bp;

    pcf = *cf;
    cf->ctx = ctx;
    cf->cmd_type = NGX_HTTP_UPS_BP_CONF;
    ...
    *cf = pcf;
    ...
    if (bp->max_retries == NGX_CONF_UNSET_UINT) {
        bp->max_retries = 1;
    }

    if (bp->peers.nelts == 0) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "no bind addresses inside upstream_bind_pool");
        return NGX_CONF_ERROR;
    }
...
static char *
ngx_http_upstream_bind_pool_bind(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_http_upstream_bind_pool_t  *bp = conf;

    ngx_str_t                       *value, s;
    ngx_url_t                        u;
    ngx_http_upstream_bind_peer_t   *peer;
    ngx_uint_t                       i;

    value = cf->args->elts;

    peer = ngx_array_push(&bp->peers);
    if (peer == NULL) {
        return NGX_CONF_ERROR;
    }

    ngx_memzero(peer, sizeof(ngx_http_upstream_bind_peer_t));
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "max_fails=", 10) == 0) {
            ...
            peer->max_fails = ngx_atoi(s.data, s.len);

            if (peer->max_fails == (ngx_uint_t) NGX_ERROR) {
                goto invalid;
            }
        ...
        if (ngx_strncmp(value[i].data, "fail_timeout=", 13) == 0) {
            ...
            peer->fail_timeout = ngx_parse_time(&s, 1);

            if (peer->fail_timeout == (time_t) NGX_ERROR) {
                goto invalid;
            }
...
ngx_http_upstream_create_main_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_array_init(&umcf->bind_pools, cf->pool, 4,
                       sizeof(ngx_http_upstream_bind_pool_t *))
        != NGX_OK)
    {
        return NULL;
    }
