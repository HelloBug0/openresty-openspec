...
static ngx_command_t  ngx_http_proxy_commands[] = {
    ...
    { ngx_string("proxy_bind_pool"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_proxy_loc_conf_t, upstream.bind_pool_name),
      NULL },
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_str_value(conf->upstream.bind_pool_name,
                              prev->upstream.bind_pool_name, "");

    if (conf->upstream.bind_pool_name.len) {
        ngx_http_upstream_main_conf_t *umcf;
        ngx_http_upstream_bind_pool_t **bpp;
        ngx_uint_t i;

        umcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_upstream_module);
        bpp = umcf->bind_pools.elts;

        for (i = 0; i < umcf->bind_pools.nelts; i++) {
            if (bpp[i]->name.len == conf->upstream.bind_pool_name.len &&
                ngx_strncmp(bpp[i]->name.data, conf->upstream.bind_pool_name.data,
                            conf->upstream.bind_pool_name.len) == 0)
            {
                conf->upstream.bind_pool = bpp[i];
                break;
            }
        }

        if (conf->upstream.bind_pool == NULL) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                               "upstream bind pool \"%V\" not found",
                               &conf->upstream.bind_pool_name);
