...
- [x] 3.1 Build OpenResty with the patch applied.
- [x] 3.2 Run Nginx with `SSLKEYLOGFILE` set and verify the file is created and populated.
- [x] 3.3 Verify Nginx behavior when `SSLKEYLOGFILE` is NOT set (no file created).
- [x] 3.4 Verify Nginx behavior when `SSLKEYLOGFILE` points to an invalid path (error logged, Nginx starts).
- [x] 3.5 Use Wireshark or `tshark` to decrypt captured traffic using the generated key log.
